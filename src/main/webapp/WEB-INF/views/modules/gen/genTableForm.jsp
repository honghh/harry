		...
		<c:when test="${empty genTable.name}">
			<form:form id="inputForm" modelAttribute="genTable" action="${ctx}/gen/genTable/form" method="post" class="form-horizontal">
				...
				<div class="control-group">
					...
					<div class="controls">
						<form:select path="name" class="form-control  max-width-300 input-xxlarge">
							<form:options items="${tableList}" itemLabel="nameAndComments" itemValue="name" htmlEscape="false"  cssClass="form-control  max-width-250 required input-xlarge" />
						</form:select>
					</div>
					<br/>
			...
			<form:form id="inputForm" modelAttribute="genTable" action="${ctx}/gen/genTable/save" method="post" class="form-horizontal">
				...
				<fieldset>
					...
					<div class="control-group">
						<label class="col-sm-3 control-label">表名:</label>
						<div class="controls">
							<form:input path="name" htmlEscape="false" maxlength="200" class="form-control  max-width-250 required" readonly="true"/>
						</div>
					</div>
					<div class="control-group">
						<label class="col-sm-3 control-label">说明:</label>
						<div class="controls">
							<form:input path="comments" htmlEscape="false" maxlength="200" class="form-control  max-width-250 required"/>
						</div>
					</div>
					<div class="control-group">
						<label class="col-sm-3 control-label">类名:</label>
						<div class="controls">
							<form:input path="className" htmlEscape="false" maxlength="200" class="form-control  max-width-250 required"/>
						</div>
					</div>
					<div class="control-group">
						<label class="col-sm-3 control-label">父表表名:</label>
						<div class="controls">
							<form:select path="parentTable" cssClass="form-control  max-width-250 input-xlarge">
								<form:option value="">无</form:option>
								<form:options items="${tableList}" itemLabel="nameAndComments" itemValue="name" htmlEscape="false"/>
							</form:select>
							&nbsp;当前表外键：
							<form:select path="parentTableFk" cssClass="form-control  max-width-250 input-xlarge">
								<form:option value="">无</form:option>
					...
					<div class="control-group hide">
						...
						<div class="controls">
							<form:textarea path="remarks" htmlEscape="false" rows="4" maxlength="200" class="form-control  max-width-250 input-xxlarge"/>
						</div>
					</div>
					<legend>字段列表</legend>
					<div class="control-group">
						<table id="contentTable" class="table table-striped table-bordered table-condensed">
							...
							<c:forEach items="${genTable.columnList}" var="column" varStatus="vs">
								<tr${column.delFlag eq '1'?' class="error" title="已删除的列，保存之后消失！"':''}>
									...
									<td>
										<input type="text" name="columnList[${vs.index}].comments" value="${column.comments}" maxlength="200" class="form-control  max-width-250 required" style="width:100px;"/>
									</td>
									<td nowrap>
										<input type="hidden" name="columnList[${vs.index}].jdbcType" value="${column.jdbcType}"/>${column.jdbcType}
									</td>
									<td>
										<select name="columnList[${vs.index}].javaType" class="form-control  max-width-250 required input-mini" style="width:85px;*width:75px">
									...
									<td>
										<input type="text" name="columnList[${vs.index}].javaField" value="${column.javaField}" maxlength="200" class="form-control  max-width-250 required input-small"/>
									</td>
									<td>
									...
									<td>
										<select name="columnList[${vs.index}].queryType" class="form-control  max-width-250 required input-mini">
									...
									<td>
										<select name="columnList[${vs.index}].showType" class="form-control  max-width-250 required" style="width:100px;">
									...
									<td>
										<input type="text" name="columnList[${vs.index}].dictType" value="${column.dictType}" maxlength="200" class="form-control  max-width-250 input-mini"/>
									</td>
									<td>
										<input type="text" name="columnList[${vs.index}].sort" value="${column.sort}" maxlength="200" class="form-control  max-width-250 required input-min digits"/>
									</td>
								</tr>
							</c:forEach>
							</tbody>
