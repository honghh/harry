 ...
 */
package com.qd.modules.gen.util;
...
import com.qd.common.mapper.JaxbMapper;
import com.qd.common.utils.DateUtils;
import com.qd.common.utils.FileUtils;
import com.qd.common.utils.FreeMarkers;
import com.qd.common.utils.StringUtils;
import com.qd.modules.gen.entity.GenCategory;
import com.qd.modules.gen.entity.GenConfig;
...
public class GenUtils {
	...
	public static void initColumnField(GenTable genTable) {
		for (GenTableColumn column : genTable.getColumnList()) {
			...
			if (StringUtils.startsWithIgnoreCase(column.getJdbcType(), "CHAR")
				...
				column.setShowType("dateselect");
					...
					|| StringUtils.startsWithIgnoreCase(column.getJdbcType(),
							"NUMBER")) {
				// 如果是浮点型
				String[] ss = StringUtils.split(StringUtils.substringBetween(
						column.getJdbcType(), "(", ")"), ",");
			...
			if (StringUtils.equalsIgnoreCase(column.getName(), "name")
					...
					|| StringUtils
							.equalsIgnoreCase(column.getName(), "remarks")
					|| StringUtils.equalsIgnoreCase(column.getName(),
							"update_date")) {
				column.setIsList("1");
			}
			...
			if (StringUtils.startsWithIgnoreCase(column.getName(), "user_id")) {
				...
				column.setJavaField(column.getJavaField().replaceAll("Id",
						".id|name"));
			...
			// 备注、内容
					...
					|| StringUtils
							.equalsIgnoreCase(column.getName(), "content")) {
				column.setShowType("textarea");
			}
			// 父级ID
			else if (StringUtils
					.equalsIgnoreCase(column.getName(), "parent_id")) {
	...
	public static Map<String, Object> getDataModel(GenScheme genScheme) {
		...
		model.put("className", StringUtils.uncapitalize(genScheme.getGenTable()
				.getClassName()));
		model.put("ClassName",
		...
		model.put("functionAuthor", StringUtils.isNotBlank(genScheme
				.getFunctionAuthor()) ? genScheme.getFunctionAuthor()
		...
		model.put(
				...
				model.get("moduleName")
						+ (StringUtils.isNotBlank(genScheme.getSubModuleName()) ? "/"
								+ StringUtils.lowerCase(genScheme
										.getSubModuleName())
								: "") + "/" + model.get("className"));
	...
	public static String generateToFile(GenTemplate tpl,
		...
		String fileName = Global.getProjectPath()
				+ File.separator
				+ StringUtils.replaceEach(FreeMarkers.renderString(
						tpl.getFilePath() + "/", model), new String[] { "//",
						"/", "." }, new String[] { File.separator,
		...
		String content = FreeMarkers.renderString(
				StringUtils.trimToEmpty(tpl.getContent()), model);
