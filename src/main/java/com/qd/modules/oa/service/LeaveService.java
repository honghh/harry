...
public class LeaveService extends BaseService {
	...
	public void save(Leave leave, Map<String, Object> variables) {
		...
		identityService.setAuthenticatedUserId(leave.getCurrentUser()
				.getLoginName());
		...
		ProcessInstance processInstance = runtimeService
				.startProcessInstanceByKey(ActUtils.PD_LEAVE[0], businessKey,
						variables);
	...
	public List<Leave> findTodoTasks(String userId) {
		...
		for (Task task : tasks) {
			...
			leave.setTask(task);
			leave.setProcessInstance(processInstance);
			leave.setProcessDefinition(repositoryService
